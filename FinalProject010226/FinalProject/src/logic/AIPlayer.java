...
public class AIPlayer {
    ...
    private void buildStrategyTree() {
        ...
        // שאלה: אמצע משחק? כן→הגנה, לא→סיום
        DecisionNode isMidgameNode  = new IsMidgameNode(defenseAction, endgameAction);

        // פתיחה → אם נגמרה: ממשיך לaמצע
        DecisionNode openingAction  = new OpeningActionNode(isMidgameNode);
        // שאלה שורש: פתיחה? כן→openingAction, לא→isMidgame
        DecisionNode isOpeningNode  = new IsOpeningNode(openingAction, isMidgameNode);
    ...
    // בחר את המהלך שנותן את ניקוד ה-evaluate הגבוה ביותר
    ...
    public int evaluate(GameState state) {
        int score = 0;
        // קבע את שלב המשחק לצורך בחירת הטבלאות הנכונות
