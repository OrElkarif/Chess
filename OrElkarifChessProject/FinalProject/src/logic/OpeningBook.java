...
public class OpeningBook {//ספר פתיחות קבוע
    ...
    public Move getNextMove(GameState state, Evaluator evaluator) {//שולפת מהספר את המהלך המדויק
        ...
        for (Byte pos : state.getWhiteQueens()) {//אם המלכה הלבנה עברה את החצי, סיים פתיחה
            if (GameState.getRowFromByte(pos) <= BOARD_SIZE/2) {
                openingDone = true; return null;
            }
        }

        if (openingIndex >= currentOpening.length) {// האם סיימנו את המהלכים 
            openingDone = true; return null;
        }
