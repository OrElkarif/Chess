...
public class Pawn extends Piece implements java.io.Serializable {//מייצגת רגלי פשוט
    private static final long serialVersionUID = 1L;
	public boolean _havePawnMoved= false;
	
	public Pawn( int col, int row, Color color) {//שם אותו במשבצת עם צבע וטוען תמונת רגלי
		super(col,row,color);
	...
	@Override
	public boolean isValidMovement(int fromRow, int fromCol, int toRow, int toCol, Piece eaten) {//בודקת אם הוא צעד אחד קדימה אולי שניים ואם זה תקין לאכילה באלכסון
	...
	@Override
	public boolean isEnPassant(GameState state, int fromRow, int fromCol, int toRow, int toCol, Move move) {//אכילה אלכסונית כשיש מישהו לידו שעשה שני צעדים
	...
	@Override 
	public void haveMoved(GameState state) {//אם הוא כבר זז הוא לא יכול יותר ללכת שני צעדים קדימה
		_havePawnMoved=true;
		state._resetCounter=true;
	}
