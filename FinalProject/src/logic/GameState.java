...
//רשימת מיקומים של החיילים - 4 ספרות ימניות זה עמודה  , 4 שמאליות זה שורה
	...
	public boolean isValidMovement(Move move) {//בודקת מול הכלי עצמו האם התנועה חוקית בהתאם לחוקים שלו
	...
	public boolean isThereCheckmate(Color color) {//בודקת אם שחקן לא יכול לזוז לשום מקום חוקי בזמן שהמלך שלו תחת שח
		...
		for (byte pieceType : pieceTypes) {
		    ...
		    for (Byte position : positionsCopy) {
		        ...
		        if (!p._allPossibleMoves.isEmpty()) {
		            return false; 
		        }
		    }
		}
