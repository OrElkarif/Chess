...
public class Evaluator {//מחלקה שמעריכה כמה הלוח טוב לכל שחקן ונותנת ציון מספרי כדי שהבוט ידע מה לבחור
    ...
    private int calculateThreatScore(GameState state) {//מוסיפה או מורידה נקודות על כלים שלנו שמאוימים עכשיו ויכולים להיאכל
        int threatScore = 0;
    ...
    private int passedPawnScore(GameState state) {//רגלי שיכול לרוץ עד הסוף ולהפוך למלכה בלי שאף אחד מפריע לו
        int score = 0;
    ...
    private int pawnChainScore(GameState state) {//מזהה שרשראות רגלים
        int score = 0;
        for (Byte pos : state.getBlackPawns()) {
            int r = GameState.getRowFromByte(pos);
            int c = GameState.getColFromByte(pos);
