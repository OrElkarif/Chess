...
public class Move {
	...
	public void DoThisMove(GameState state) {
	    ...
	    if(_isPromotion && _promoteTo != null) {
	        state.get_board()[_toRow][_toCol] = _promoteTo;
	        _promoteTo.setRow(_toRow);
	        _promoteTo.setCol(_toCol);
	        
	        // ---> התיקון: עדכון הרשימות כשיש הכתרה <---
	        state.removePieceFromList(_piece, _toRow, _toCol); // מחיקת הרגלי (רוח הרפאים)
	        state.addPieceToList(_promoteTo, _toRow, _toCol); // הוספת המלכה/כלי חדש לרשימה
	    }
	}
