...
public class AIPlayer {
        ...
        for (Move move : moves) {
            ...
            if (whiteAttacker != null) {
                ...
                if (blackDefender != null) {
                    // אם המשבצת מוגנת, בדוק אם החילוף משתלם
                    if (myValue > attackerValue) {
                        // אם אני מלכה (900) והוא סוס (300), זה חילוף נורא. 
                        // נוריד את כל הערך של המלכה מהציון כדי למנוע את המהלך.
                        score -= myValue; 
                    }
                } else {
                    // אם המשבצת לא מוגנת בכלל - זה מהלך התאבדות.
                    // נוריד קנס כפול אם מדובר במלכה!
                    if (move.get_piece().getPieceType() == Piece.QUEEN) {
                        score -= (myValue * 2); 
                    } else {
                        score -= myValue;
                    }
 ...
 // 1. חישוב "איום מרחבי" - מוריד ניקוד אם הכלים שלנו מאוימים
            ...
            for (int c = 0; c < state.get_board()[r].length; c++) {
                ...
                if (p != null) {
                    ...
                    if (p.getColor() == Color.black) { // כלי שלנו
                        if (isSquareUnderAttack(state, r, c, Color.black)) { // לבן מאיים עליו
                            ...
                            if (!isSquareDefended(state, r, c, Color.black)) {
                                threatScore -= pieceValue; // כלי שלנו בסכנה וללא הגנה
                            } else {
                                // התיקון: הכלי מוגן, אבל אם התוקף שווה פחות מאיתנו (למשל רגלי תוקף מלכה)
                                if (attackerVal > 0 && attackerVal < pieceValue) {
                                    threatScore -= pieceValue; // קנס עצום! חייבים לברוח מיד!
                                } else {
                                    threatScore -= (pieceValue / 10); // מוגן וחילוף שווה, רק קנס קטן על הלחץ
                                }
                            }
                        }
                    } else { // כלי של הלבן
                        if (isSquareUnderAttack(state, r, c, Color.white)) { // אנחנו מאיימים עליו
                            Piece attacker = getLowestValueAttacker(state, r, c, Color.black);
                            int attackerVal = (attacker != null) ? getPieceValue(attacker.getPieceType()) : 0;

                            if (!isSquareDefended(state, r, c, Color.white)) {
                                threatScore += pieceValue; // בונוס!
                            } else {
                                if (attackerVal > 0 && attackerVal < pieceValue) {
                                    threatScore += pieceValue; // בונוס עצום! אנחנו תוקפים כלי יקר שלו עם כלי זול שלנו!
                                } else {
                                    threatScore += (pieceValue / 10); 
                                }
                            }
                        }
                    }
