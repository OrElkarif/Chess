...
public class BoardPanel extends JPanel {
    ...
    // הכפתור שמופיע כאשר המשחק נגמר – מעל הלוח
    private JButton _viewResultButton;

    public BoardPanel() {
        setLayout(null); // פריסה חופשית כדי לאפשר מיקום מוחלט לכפתור
        Player white = new Player(Color.white, true);
        Player black = new Player(Color.black, true);
        _gameLogic = new GameLogic(white, black);
        setPreferredSize(new java.awt.Dimension(800, 800));

        // יצירת הכפתור מראש – הוא יוצג רק בסוף המשחק
        ...
        _viewResultButton.setVisible(false); // מוסתר עד סוף המשחק
        _viewResultButton.setBounds(275, 370, 250, 60); // מרכז הלוח
        ...
        _viewResultButton.addActionListener(e -> {
            ...
            if (frame != null) {
                ...
                // ---> התיקון: שולפים את הסיבה מה-GameState <---
                String reason = _gameLogic._state.getEndGameReason(); 
                
                frame.getContentPane().removeAll();
                // ---> התיקון: מעבירים את הסיבה כפרמטר למסך הניצחון <---
    ...
    private void executeMove(int fromRow, int fromCol, int toRow, int toCol) {
        ...
        boolean isPromotion = (piece != null
                && piece.getPieceType() == Piece.PAWN
                && ((piece.getColor() == Color.white && toRow == 0)
    ...
    private void checkGameOver() {
        if (_gameLogic._state.is_isCheckmate() || _gameLogic._state.is_isStalemate()) {
            ...
            // מציג את הכפתור מעל הלוח במקום לנווט מיד
    ...
    public void paintComponent(Graphics g) {
        ...
        // אם המשחק נגמר – מציירים שכבת כהה שקופה מעל הלוח
        if (!_gameLogic._gameInProcess) {
