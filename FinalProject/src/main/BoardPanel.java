...
public class BoardPanel extends JPanel {
    ...
    private void executeMove(int fromRow, int fromCol, int toRow, int toCol) {
        ...
        // בדיקה מדויקת של כיווני ההכתרה
        boolean isWhitePromotion = (piece.getColor() == Color.white && toRow == 0);
        boolean isBlackPromotion = (piece.getColor() == Color.black && toRow == BOARD_SIZE - 1);

        if (piece.getPieceType() == Piece.PAWN && (isWhitePromotion || isBlackPromotion)) {
            if (piece.getColor() == Color.black) {
                // הבוט הגיע לשורה 7 - הכתרה אוטומטית למלכה
                Piece promotedPiece = new Queen(toCol, toRow, Color.black);
