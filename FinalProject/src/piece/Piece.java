...
public abstract class Piece implements java.io.Serializable {//מחלקת האב הכללית שכל כלי שחמט יורש ממנה את הדברים שעושים אותו כלי
	...
	public Piece(int col, int row, Color color) {//מקבל int לנוחות, שומר כbyte
		this.col = (byte) col;
		this.row = (byte) row;
		this.color = color;
	}
	...
	public abstract boolean isMoveOverAnotherPiece(GameState state,int fromRow, int fromCol, int toRow, int toCol); //מופשטת מכיוון שהיא לא תקפה לסוס
	
	
	
	
    public void MakeMovement(GameState state, Move move) {//מעדכנת נטו את המיקום הפנימי של הכלי לאן שהוא הגיע
    	Piece[][] board = state.get_board();//מצביע חדש לboard
...
public abstract byte getPieceType();
	...
	public void setCol(int col) { this.col = (byte) col; }//int לנוחות call sites, cast פנימי
	public byte getRow() { return row; }
	public void setRow(int row) { this.row = (byte) row; }
