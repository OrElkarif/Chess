...
public class King extends Piece{
	...
	public boolean isHazrahaLegal(GameState state, int fromRow, int fromCol, int toRow, int toCol) {
	 ...
	 // ---> התיקון: בדיקה מפורשת שכל המשבצות בין המלך לצריח ריקות לחלוטין <---
	    if (isKingSide) {
	        // צד מלך (הצרחה קטנה): משבצות 5 ו-6 חייבות להיות ריקות
	        if (state.getPiecePlace(fromRow, 5) != null || state.getPiecePlace(fromRow, 6) != null) {
	            return false;
	        }
	    } else {
	        // צד מלכה (הצרחה גדולה): משבצות 1, 2 ו-3 חייבות להיות ריקות
	        if (state.getPiecePlace(fromRow, 1) != null || 
	            state.getPiecePlace(fromRow, 2) != null || 
	            state.getPiecePlace(fromRow, 3) != null) {
	            return false;
	        }
	    }
